...
class ApiSpec extends FlatSpec with ShouldMatchers with OptionValues with Inside with BeforeAndAfterAll {
  ...
  override def beforeAll(configMap: Map[String, Any]) {
    ...
    val options = UploadParameters(
      publicId = "api_test",
      tags = List("api_test_tag"),
      eager = List(Transformation().w_(100).c_("scale")))
    Await.result(for {
      r1 <- api.deleteResources(List("api_test", "api_test1", "api_test2", "api_test3", "api_test_by_prefix")).recover { case _ => "r1 failed" }
      ...
      r6 <- cloudinary.uploader().upload("src/test/resources/logo.png", options.copy(publicId = "api_test1")).recover { case _ => "r6 failed" }
  ...
  it should "allow listing resources by type" in {
    Await.result(api.resources(`type` = "upload").map(response => response.resources.map(_.public_id).toSet), 5 seconds) should contain("api_test")
  }

  it should "allow listing resources by prefix" in {
    Await.result(api.resources(`type` = "upload", prefix = "api_test").map(response => response.resources.map(_.public_id).toSet), 5 seconds) should equal(Set("api_test", "api_test1"))
  }

  it should "allow listing resources by tag" in {
    Await.result(api.resourcesByTag("api_test_tag").map(response => response.resources.map(_.public_id).toSet), 5 seconds) should equal(Set("api_test", "api_test1"))
  }
  ...
  it should "allow deleting resources by tags" in {
    Await.result(for {
      r1 <- cloudinary.uploader().upload("src/test/resources/logo.png", UploadParameters(publicId = "api_test4", tags = List("api_test_tag_for_delete")))
