...
import com.cloudinary.response.{CustomCoordinate, UploadResponse}
...
class SearchSpec extends MockableFlatSpec with Matchers with OptionValues with Inside with BeforeAndAfterAll {
  ...
  override def beforeAll(): Unit = {
    ...
    val (r1, r2, r3) = Await.result(for {
      r1 <- cloudinary.uploader().upload(s"$testResourcePath/logo.png", options.publicId(searchTag + "1")).map(_.public_id).recover { case _ => "r1 failed" }
      r2 <- cloudinary.uploader().upload(s"$testResourcePath/logo.png", options.publicId(searchTag + "2")).map(_.public_id).recover { case _ => "r2 failed" }
      r3 <- cloudinary.uploader().upload(s"$testResourcePath/logo.png", options.publicId(searchTag + "3")).map(_.public_id).recover { case _ => "r3 failed" }
